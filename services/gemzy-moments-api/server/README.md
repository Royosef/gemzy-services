...
2. Create a storage bucket dedicated to Gemzy Moments collections.
...
| `GCS_COLLECTIONS_APP_BUCKET`    | Yes      | Private bucket that stores user collection images. |
| `GCS_COLLECTIONS_PUBLIC_BUCKET` | Yes      | Public bucket used for avatars and model artwork. |
...
the migration seeds the Free, Pro, and Designer plans with 100, 500, and 1000
credits respectively. Update those rows to change allocations without
deploying new code; the FastAPI process caches the values for a few minutes and
...
Build from `services/gemzy-moments-api/server`:

```sh
docker build -t gemzy-moments-server -f ../Dockerfile ../..
```
...
docker run -d \
  --env-file ../.env \
  -p 5050:5050 \
  --name gemzy-moments-server \
  --network gemzy-net \
  gemzy-moments-server
```
...
```sh
docker logs -f gemzy-moments-server
```

Lifecycle:

```sh
docker stop gemzy-moments-server
docker start gemzy-moments-server
docker restart gemzy-moments-server
docker rm gemzy-moments-server
docker rmi gemzy-moments-server
docker exec -it gemzy-moments-server bash
docker ps
docker ps -a
```
